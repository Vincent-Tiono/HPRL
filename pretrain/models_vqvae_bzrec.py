...
from quantized_fn import vq, vq_st
...
class ProgramEncoder(NNBase):
    def __init__(self, num_inputs, num_outputs, recurrent=True, hidden_size=64, rnn_type='GRU', two_head=False, dropout=0.0, use_linear=False, unit_size=256):
        ...
        self.token_encoder = nn.Embedding(num_inputs, num_inputs) #(size of vocab, dim of each embedding vector)
        self._use_linear = use_linear
...
class VQEmbedding(nn.Module):
    ...
    def straight_through(self, z_e_x):
        #z_e_x: encoder output
        z_e_x_ = z_e_x.permute(0, 2, 3, 1).contiguous()
        
        # method 1: quantized_vectors (after codebook), indices
        # use VectorQuantizationStraightThrough.apply
        z_q_x_, indices = vq_st(z_e_x_, self.embedding.weight.detach())
        z_q_x = z_q_x_.permute(0, 3, 1, 2).contiguous()

        # method 2: get codebok vectors directly
