...
def encode_programs(program_data, program_encoder, dsl):
    ...
    for program_id, program_code in program_data:
        try:
            ...
            with torch.no_grad():
                ...
                encoder_output = program_encoder(tokens_tensor, src_len)
                
                # Handle different return types from the encoder
                if isinstance(encoder_output, torch.nn.utils.rnn.PackedSequence):
                    # Unpack the sequence to get the last hidden state
                    unpacked_seq, unpacked_len = torch.nn.utils.rnn.pad_packed_sequence(encoder_output)
                    
                    # For each item in the batch (only 1 in this case), get the last valid hidden state
                    batch_size = unpacked_seq.size(1)  # should be 1
                    
                    # Create a tensor to hold final hidden states
                    final_states = []
                    
                    for i in range(batch_size):
                        # Get the last valid index for this sequence
                        last_idx = unpacked_len[i] - 1
                        # Extract the final hidden state
                        final_state = unpacked_seq[last_idx, i]
                        final_states.append(final_state)
                    
                    # Stack the final states into a batch
                    h_n = torch.stack(final_states)
                
                elif isinstance(encoder_output, tuple):
                    # If it's a tuple, assume it contains (output, hidden_state)
                    # and take the hidden state
                    h_n = encoder_output[1]
                    if isinstance(h_n, tuple):  # For LSTM which returns (h_n, c_n)
                        h_n = h_n[0]
                    
                    # If the hidden state is stacked (layers, batch, hidden),
                    # take the last layer
                    if len(h_n.shape) == 3:
                        h_n = h_n[-1]
                ...
                latent_vectors.append(h_n.cpu().numpy())
                program_ids.append(program_id)
    ...
    if programs_with_encoding > 0:
        multiplication_factor = len(latent_vectors) / programs_with_encoding
        print(f"Multiplication factor: {multiplication_factor:.2f}x (should be 1.0)")
