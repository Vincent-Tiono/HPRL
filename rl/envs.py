...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, custom_env=False, custom_env_type='program',
    ...
    def _custom_program_thunk():
        ...
        if custom_kwargs['config'].mdp_type == 'ProgramEnv1_new_vae_v2':
            env = ProgramEnv1_new_vae_v2(**custom_kwargs)
        elif custom_kwargs['config'].mdp_type == 'ProgramEnv_option_new_vae_v2_key2door_fixed':
            env = ProgramEnv_option_new_vae_v2_key2door_fixed(**custom_kwargs)
        else:
            raise NotImplementedError()
